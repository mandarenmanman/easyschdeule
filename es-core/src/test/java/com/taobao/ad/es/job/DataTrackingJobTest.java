...
public class DataTrackingJobTest {
	...
	DataTrackingJob job = null;

	@Before
	public void setUp() {
		// 启动测试的http任务
		JobHttpServerForTest.StartServer();
		job = new DataTrackingJob();
		jobDetail = new JobDetail("jobName", "123", DataTrackingJob.class);
		jobDetail.setDescription("Http－测试");
		jobDetail.setRequestsRecovery(false);
		jobDetail.setDurability(true);
		List<CodeDO> list = codeBO.getCodesByKey(CodeDO.CODE_KEY_DTJOB_JOBDATA);
	...
	public void testDoJob_正常() {
		JobResult jobResult = job.doJob(1234L, jobDetail, TARGET + JobHttpServerForTest.QUERYPATH_TEST_DTJOB_RETURNTRUE);
		Assert.assertEquals(true, jobResult.isSuccess());
	}

	//@Test
	public void testDoJob_正常_忽略任务结果检查() {
		jobDetail.getJobDataMap().put(Constants.JOBDATA_CHECKRESULT, "false");
		JobResult jobResult = job.doJob(1234L, jobDetail, TARGET + JobHttpServerForTest.QUERYPATH_TEST_HTTPJOB_RETURNERROR);
		Assert.assertEquals(true, jobResult.isSuccess());
		Assert.assertEquals(JobResult.RESULTCODE_JOBRESULT_IGNORE, jobResult.getResultCode());
	}

	@Test
	public void testDoJob_异常() {
		JobResult jobResult = job.doJob(1234L, jobDetail, TARGET + JobHttpServerForTest.QUERYPATH_TEST_DTJOB_RETURNFALSE);
