 ...
 * @author baimei
 */
public class StoredProcedureJobServlet extends HttpServlet {

	private static final long serialVersionUID = 1L;
	final Logger logger = LoggerFactory.getLogger(StoredProcedureJobServlet.class);

	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
		response.setContentType("application/json;charset=utf-8");
		PrintWriter out = response.getWriter();
		JobExecutor executor = new StoredProcedureJobExecutor(WebAgentConstants.DATASOURCE_CONTEXT_PATH);
